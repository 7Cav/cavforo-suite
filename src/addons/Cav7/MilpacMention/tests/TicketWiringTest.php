...
// XF writes its own mention alerts (the ticket surface inherits ['autoRead' => false] from
// XF\Notifier\AbstractNotifier). insertAlert() reads autoRead out of the OPTIONS array
// (arg 8) and defaults it to true when the array omits it, so a milpac_mention row without
// the flag saves auto_read=1 and clears on a different schedule than the @-mention it
// mirrors (spec §2.5). depends_on_addon_id lives in the EXTRA array (arg 7); the two are
// separate slots. Pin the tail of the real ->alert(...) call POSITIONALLY on the
// comment-stripped $code: action, then the extra array carrying depends_on_addon_id, then
// ['autoRead' => false] as the last argument. Folding autoRead into the extra array,
