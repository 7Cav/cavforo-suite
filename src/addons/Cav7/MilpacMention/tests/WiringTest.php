...
//
// Two hardenings (issue #100 review):
...
check(
    ...
    'Post fires inline-only (a resumed job\'s stash is empty), so it must contain the pre-loop findByIds/repository lookups in an outer guard like Report, with the outer catch(\\Throwable) forwarding to logException — a transient DB fault there would otherwise 500 the already-committed post action and drop every milpac recipient (issue #100); removing the outer guard, dropping its logException, or narrowing it to \\Exception FAILS this'
);
