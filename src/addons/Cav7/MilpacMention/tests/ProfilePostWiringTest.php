...
foreach ($surfaces as $name => $s) {
    ...
    // Anchor to the comment-stripped body ($code) so a prose "try"/"catch" in a comment
    // cannot register as a real keyword in the positional pin below (mirrors
    // TicketWiringTest).
    ...
    // are inside the guard, nor that the OUTER catch (not just the inner per-recipient
    // one) forwards to logException. This surface fires inline with no deferred-job
    // net, so a DB error on findByIds would 500 the already-committed action unless it
    // too is contained. Pin the layout: the outer try opens BEFORE findByIds, findByIds
    // precedes the loop, and the outer catch(\Throwable)->logException follows the loop.
    //
    ...
    //    the outer catch onward ($outerCatchRegion), not across the whole body. The
    //    inner per-recipient catch already forwards to logException, so a whole-body
    //    scan let a silent-swallow mutation of the OUTER catch (dropping its
    //    logException, or narrowing \Throwable to \Exception) pass while the inner catch
    //    satisfied the pattern. Anchored to the outer catch, that swallow goes red.
    ...
    check(
        "$name: containment is report-style — the outer try opens BEFORE findByIds, findByIds precedes the loop, and the outer catch(\\Throwable)->logException follows the loop",
