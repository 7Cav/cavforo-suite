        ...
        try
        {
            ...
            foreach ($milpacUserIds as $userId)
            {
                ...
                try
                {
                    ...
                    // they view the alert itself, exactly as XF's own mention alerts do (the
                    // stock ProfilePost notifier passes the same flag); without it insertAlert()
                    // defaults auto_read=1 and the alert clears on a different schedule than the
                    // @-mention it mirrors. depends_on_addon_id stays in the $extra array —
                    // insertAlert() reads the two from separate slots.
