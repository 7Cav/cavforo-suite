...
2. Rebuild the installable data with `php cmd.php xf-addon:build Cav7/<AddonId>`. This compiles `_output/` into `_data/`.
3. Commit both `_output/` and the regenerated `_data/`. Do not hand-edit `_data/`.
...
3. Fill in `addon.json`: title, description, `addon_id`, `namespace`, version, and any dependencies in `require`.
4. Build and install it, then develop as above.
5. Add a short `README.md` describing what the addon does and what it requires.

## Build, test, and release

The shared build, test, and release tooling is being set up in #10. Until it lands, build addons locally with `xf-addon:build` and `xf-addon:build-release` as described above. Release tags are per addon and use `<AddonId>-vX.Y.Z`.
