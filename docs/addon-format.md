...
- `README.md`: what the addon is, how to run it, how to use it, and a provenance note (see below). What earns a place in one is in [CONTRIBUTING.md](../CONTRIBUTING.md#what-goes-in-an-addon-readme).
...
- Give `title`, `version_id` and `version_string` a usable value, not just the key. `null` is not one, and XenForo will not catch it: `xf-addon:validate-json` tests these three with `isset()`, so it reports a null as *missing* rather than as the wrong type, offers to repair it, and then leaves it exactly where it was — the repair is `array_replace($defaults, $json)`, which fills an absent key but not a null one. Having "repaired" nothing it treats the errors as resolved and exits 0, a hole XenForo's own comment on that branch concedes. [`tools/validate-addon.php`](../tools/validate-addon.php) refuses a null in any of the three, and refuses an empty `title` as well, which is stricter than XenForo: `is_string()` accepts `''`, but an add-on the admin panel lists under a blank name is nobody's intent. A title of spaces is a rule about content and is not checked.
