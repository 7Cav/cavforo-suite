...
API to admin users, which blocks group-based RBAC for satellite services
...
Instead the add-on registers a dedicated `user:groups` scope, matching how our
other in-house add-ons extend the API (`donation:read`, `nf_discord:read`):
