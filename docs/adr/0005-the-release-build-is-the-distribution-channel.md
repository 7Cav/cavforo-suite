...
- Every entry in the `excludes` array is asserted, by one of three rules: it is a dev-only path, a build input, or prefixed. The three exist because the categories are genuinely different — a shipped `tests/*.php` is an executable endpoint, a shipped `build.json` is inert — but the assertion does not grade on that. An entry in the array with nothing asserting it is the defect this ADR is a response to, at one add-on's scale.
