...
- Every entry in the `excludes` array falls under one of three rules: it is a dev-only path, a build input, or prefixed. The three exist because the categories are genuinely different — a shipped `tests/*.php` is an executable endpoint, a shipped `build.json` is inert — but the rules do not grade on that. An entry in the array with no rule reaching it is the defect this ADR is a response to, at one add-on's scale.
- Covered by a rule is not the same as exercised. A rule fails a build only where an add-on carries the path, so `.gitattributes` is covered and unexercised — no add-on has one — and `build.json` is exercised only because MilpacMention does. The rules also differ in reach: dev-only paths and build inputs are caught at any depth, the prefix rule reads top-level names only. Neither is a reason to weaken a rule to what happens to be exercised today; both are reasons not to read the coverage as uniform.
